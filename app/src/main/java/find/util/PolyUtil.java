...
import static find.util.MathUtil.EARTH_RADIUS;
import static find.util.MathUtil.clamp;
import static find.util.MathUtil.hav;
import static find.util.MathUtil.havDistance;
import static find.util.MathUtil.havFromSin;
import static find.util.MathUtil.inverseMercator;
import static find.util.MathUtil.mercator;
import static find.util.MathUtil.sinFromHav;
import static find.util.MathUtil.sinSumFromHav;
import static find.util.MathUtil.wrap;
...
public class PolyUtil {
    ...
    public static boolean intersectsOrInside(Location southwest, Location northeast, Location start, Location end) {

        // inside
        if (start.getLongitude() >= southwest.getLongitude() && end.getLongitude() >= southwest.getLongitude() &&
                start.getLongitude() <= northeast.getLongitude() && end.getLongitude() <= northeast.getLongitude() &&
                start.getLatitude() <= northeast.getLatitude() && end.getLatitude() <= northeast.getLatitude() &&
                start.getLatitude() >= southwest.getLatitude() && end.getLatitude() >= southwest.getLatitude())
            return true;

        // intersects up
        if (intersects(southwest.getLongitude(), northeast.getLatitude(), northeast.getLongitude(), northeast.getLatitude(), start.getLongitude(), start.getLatitude(), end.getLongitude(), end.getLatitude()))
            return true;

        // intersects down
        if (intersects(southwest.getLongitude(), southwest.getLatitude(), northeast.getLongitude(), southwest.getLatitude(), start.getLongitude(), start.getLatitude(), end.getLongitude(), end.getLatitude()))
            return true;

        // intersects left
        if (intersects(southwest.getLongitude(), northeast.getLatitude(), southwest.getLongitude(), southwest.getLatitude(), start.getLongitude(), start.getLatitude(), end.getLongitude(), end.getLatitude()))
            return true;

        //intersects right
        if (intersects(northeast.getLongitude(), southwest.getLatitude(), northeast.getLongitude(), northeast.getLatitude(), start.getLongitude(), start.getLatitude(), end.getLongitude(), end.getLatitude()))
            return true;
    ...
    private static boolean intersects(double p0_x, double p0_y, double p1_x,
